...
class ShortformCitation(Citation):
    ...
    def as_html(self):
        ...
        inner_html = u'<span class="volume">%d</span>\\2' + \
        ...
        if self.match_url:
            ...
            span_class += " no-link"
            data_attr = ''
        return u'<span class="%s"%s><span class="antecedent">%s\\1</span>%s</span>' % (
            span_class,
            data_attr,
            self.antecedent_guess,
            inner_html
...
class SupraCitation(Citation):
    ...
    def as_html(self):
        ...
        if self.volume:
            inner_html += u'\\1<span class="volume">%d</span><span>\\2supra</span>' % self.volume
            if self.page:
                inner_html += u'<span>,\\3at\\4</span><span class="page">%s</span>\\5' % self.page
            else:
                inner_html += u'\\3'
        else:
            inner_html += u'<span>\\1supra</span>'
            if self.page:
                inner_html += u'<span>,\\2at\\3</span><span class="page">%s</span>\\4' % self.page
            else:
                inner_html += u'\\2'
...
class IdCitation(Citation):
    ...
    def as_regex(self):
        ...
        return r'%s(\s+)%s' % (
            ...
            r'(\s+)'.join([re.escape(t) for t in self.after_tokens]) + r'(\s?)'
        )
    ...
    def as_html(self):
        ...
        if self.match_url:
            ...
            span_class += " no-link"
            data_attr = ''
        return u'<span class="%s"%s>%s<span class="after_tokens">%s</span></span>' % (
            span_class,
            data_attr,
            id_token,
            ''.join([  # Backreferences must be dynamically generated
                '\\' + str(i + 1) + '<span class="after_token">' + t + '</span>'
